...
forEach(growl__closeButtons, function(index, value) {
  ...
  _el.addEventListener('click', function(e){
    // pass associated growl to method
    var _g = getClosest(e.currentTarget, '.ds-growl');
    growl__dismiss( _g );
  }, false);
});
...
function growl__show(g){
  ...
  if( growl__hasClass(_growl, 'ds-growl--centered') ){
...
function growl__dismiss(g){
  var _growl = g;
  _growl.className = _growl.className + ' ds-growl--dismissing';
...
function growl__dismissDelay(e){
  ...
  _growl.className = _growl.className.replace(' ds-growl--dismissing','');
