...
export default function () {
  ...
  forEach(modal__closeButtons, function (index, value) {
    ...
    _el.addEventListener('click', function(e){
      // pass associated modal to method
      var _m = getClosest(e.currentTarget, '.ds-modal');
      modal__close( _m );
    }, false);
  });
  ...
  function modal__show(m){
    ...
    _body.className = _body.className + ' ds-modal-scroll-fix';
  ...
  function modal__close(m){
    ...
    // Fix double scrollbar issue
    var _body = document.getElementsByTagName('body')[0];
    _body.className = _body.className.replace(' ds-modal-scroll-fix','');
